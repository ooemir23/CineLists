...
export default async function DetailsPage(props: Props) {
  ...
  const voteCount = data.vote_count
    ? data.vote_count.toLocaleString("tr-TR")
    : null;
  ...
  const statusMap: Record<string, { label: string; cls: string }> = {
    Released: {
      label: "Yayınlandı",
      cls: "text-neutral-400 border-neutral-500/20 bg-neutral-500/10",
    },
    Ended: {
      label: "Tamamlandı",
      cls: "text-neutral-400 border-neutral-500/20 bg-neutral-500/10",
        ...
        {(directors.length > 0 || creators.length > 0) && (
          <>
            ...
            {directors.slice(0, 1).map((p: any) => (
              <span key={p.id} className="text-xs text-white/60 font-bold">
                {p.name}
              </span>
            ))}
    ...
    <div className="relative min-h-screen">
      ...
      <div className="absolute top-0 left-0 right-0 h-[520px] z-0 pointer-events-none overflow-hidden">
        {(backdrop || data.poster_path) && (
          <Image
            src={
              backdrop ?? `https://image.tmdb.org/t/p/w1280${data.poster_path}`
            }
            alt=""
            fill
            sizes="100vw"
            ...
            style={{
              filter: "brightness(0.32) saturate(1.2) blur(3px)",
      ...
      <div className="relative z-10">
        ...
        <div className="flex-shrink-0 px-3.5 sm:px-6 lg:px-10 pt-1.5 md:pt-4 pb-2.5 md:pb-5 border-b border-white/[0.05]">
          ...
          <div className="md:hidden">
            ...
            <div
              className="relative w-full rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10 mb-2.5"
            ...
            >
              ...
              <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between z-10">
                <BackButton />
                <span
                  className={`flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${type === "movie" ? "bg-amber-400/20 border-amber-400/30 text-amber-400" : "bg-emerald-400/20 border-emerald-400/30 text-emerald-400"}`}
                >
                  {type === "movie" ? (
            ...
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap gap-1 items-center">
                {(data.genres || []).slice(0, 3).map((g: any) => (
                  <span
                    key={g.id}
                    className="px-2 py-0.5 rounded-full text-[10px] font-black border bg-white/5 border-white/10 text-white/60"
                  >
                    {g.name}
                  </span>
                ))}
                {statusInfo && (
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide border ${statusInfo.cls}`}
                  >
                    {statusInfo.label}
                  </span>
                )}
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-black tracking-tighter leading-tight text-white">
                  {title}
                </h1>
                {(data.original_title || data.original_name) &&
                  (data.original_title || data.original_name) !== title && (
                    <p className="text-[11px] text-white/35 italic leading-none">
                      {data.original_title || data.original_name}
                    </p>
                  )}
              </div>
              <div className="flex flex-wrap items-center gap-1.5 text-xs">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-white/35">/ 10</span>
                </div>
                {voteCount && (
                  <>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Suspense
                    fallback={
                      <span
                        className="inline-block h-8 min-w-16 rounded-lg bg-white/5 motion-safe:animate-pulse"
                ...
                {year && (
                  <>
                    <span className="text-white/20">·</span>
                    <span className="text-[11px] text-white/55 font-bold">
                      {year}
                    </span>
                  </>
                )}
                ...
                {data.number_of_seasons && (
                  <>
                    <span className="text-white/20">·</span>
                    <span className="text-[11px] text-white/55 font-bold">
                      {data.number_of_seasons} Sezon
                    </span>
                  </>
                )}
              </div>
              {data.overview && (
                <p className="text-xs text-white/65 leading-relaxed line-clamp-2 border-l-2 border-amber-400/35 pl-2.5">
                  {data.overview}
                </p>
              )}
          ...
          <div className="hidden md:flex md:items-start gap-6 lg:gap-8">
            ...
            <div className="flex-shrink-0 w-[200px] lg:w-[240px] xl:w-[280px]">
              <div
                className="relative w-full rounded-xl overflow-hidden shadow-[0_12px_40px_rgba(0,0,0,0.7)] ring-1 ring-white/10"
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-2.5">
              ...
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                    type === "movie"
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                {(data.genres || []).slice(0, 4).map((g: any) => (
                  <span
                    key={g.id}
                    className="px-2.5 py-1 rounded-full text-xs font-black border bg-white/5 border-white/10 text-white/60 hover:text-white/90 hover:border-white/20 transition-colors"
                  >
                    {g.name}
                  </span>
                ))}
                {statusInfo && (
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider border ${statusInfo.cls}`}
                  >
                    {statusInfo.label}
                  </span>
                )}
