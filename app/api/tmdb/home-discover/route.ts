...
function asDiscoverResult(
  ...
  return {
    ...
    original_title:
      typeof item.original_title === "string" ? item.original_title : undefined,
    original_name:
      typeof item.original_name === "string" ? item.original_name : undefined,
...
export async function GET(request: NextRequest) {
  ...
  const limit = Math.max(
    1,
    Math.min(Number(searchParams.get("limit") || "12"), 20),
  );
  ...
  try {
    ...
    if (timeWindow === "month" && !isDiscovering)
      discoverParams["primary_release_date.gte"] = thirtyDaysAgo;
    ...
    const fetchTypeResults = async (mediaType: "movie" | "tv") => {
      const timeParams =
        timeWindow !== "day" ? getTimeRangeParams(timeWindow) : {};

      if (currentCategory === "trending" && timeWindow !== "month") {
        return tmdb.getTrending(
          mediaType,
          timeWindow === "week" ? "week" : "day",
          { page },
        );
      }
      ...
      if (currentCategory === "upcoming") {
        ...
        if (userId) {
          ...
          const allUserWatchItems = [
            ...watching.map((item) => ({
              ...item,
              statusType: "watching" as const,
            })),
            ...toWatch.map((item) => ({
              ...item,
              statusType: "plan_to_watch" as const,
            })),
          ];
          ...
          await Promise.all(
            allUserWatchItems.map(async (item) => {
              if (item.media.type === "TV") {
                const details = await tmdb
                  .getTVShow(item.media.tmdbId.toString())
                  .catch(() => null);
                ...
                if (
                  (showStatus === "Ended" || showStatus === "Canceled") &&
                  !nextEpisode
                ) {
                  return;
                }
                ...
                let releaseDateStr = item.media.releaseDate
                  ? item.media.releaseDate.toISOString().split("T")[0]
                  : null;
                ...
                if (!releaseDateStr) {
                  const movieDetails = await tmdb
                    .getDetails("movie", item.media.tmdbId.toString())
                    .catch(() => null);
        ...
        const rawTmdbResults = (tmdbResults?.results || []) as Record<
          string,
          unknown
        >[];
      ...
      return tmdb.discover(mediaType, {
        ...discoverParams,
        ...(mediaType === "tv" && year ? { first_air_date_year: year } : {}),
      });
    };
    ...
    if (type === "all") {
      ...
      const movieResults = (movieData?.results || []) as Record<
        string,
        unknown
      >[];
    ...
    const userCountry =
      country && country.length === 2
        ? country.toUpperCase()
        : detectUserCountry(request.headers);
    const pagedResults = results.slice(0, limit);

    const resultsWithProviders =
      searchParams.get("includeProviders") === "false"
        ? pagedResults
        : await Promise.all(
            pagedResults.map(async (item) => {
              ...
              try {
                const providerData = await cachedGetWatchProviders(
                  item.media_type,
                  item.id.toString(),
                );
                const targetProviders =
                  providerData?.results?.[userCountry]?.flatrate;
