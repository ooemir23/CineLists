...
export async function HomeTopSection({
  ...
  const [trendingMovies, upcomingMovies, trendingTV, popularMovies] =
    await Promise.all([
      initialTrendingMovies
        ? Promise.resolve(initialTrendingMovies)
        : tmdb.getTrendingMovies(),
      initialUpcomingMovies
        ? Promise.resolve(initialUpcomingMovies)
        : tmdb.getUpcomingMovies(),
      initialTrendingTV
        ? Promise.resolve(initialTrendingTV)
        : tmdb.getTrendingTV(),
      initialPopularMovies
        ? Promise.resolve(initialPopularMovies)
              ...
              <div className="flex flex-col gap-1 text-center md:text-left">
                <div className="flex items-center justify-center md:justify-start gap-2">
                  ...
                  <h3 className="text-sm font-black text-white uppercase tracking-tight">
                    Kişisel Deneyimini Başlat
                  </h3>
                </div>
                <p className="text-[11px] md:text-xs text-neutral-400 font-bold max-w-sm">
                  İzlediğin dizileri takip etmek, sana özel öneriler almak ve
                  arkadaşlarınla etkileşime geçmek için hemen katıl.
                </p>
              </div>
  ...
  async function PersonalizedHero() {
    ...
    const extra: HeroItem[] = (personalized?.results || [])
      .slice(0, 3)
      .map((item) => ({
        id: Number(item.id),
      ...
      <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-3 md:gap-4">
        <div className="w-full">
          <Suspense
            fallback={
              session ? (
                <div className="h-24 rounded-3xl bg-white/5 motion-safe:animate-pulse" />
              ) : null
            }
          >
            <Upcoming />
        ...
        <div className="w-full flex-1 min-h-[320px] max-[380px]:min-h-[300px] sm:min-h-[380px] lg:min-h-0">
          <Suspense
            fallback={<HeroSlider items={items} friendPopularIds={[]} />}
          >
            <PersonalizedHero />
          </Suspense>
        </div>
      ...
      <div className="hidden lg:flex lg:col-span-5 xl:col-span-4 flex-col overflow-hidden">
        <div className="bg-[#1A202C]/60 backdrop-blur-xl rounded-[2.5rem] border border-white/5 overflow-hidden flex flex-col h-full min-h-[520px]">
          <div className="px-5 py-3 border-b border-white/5 bg-white/5 flex items-center justify-between shrink-0">
            <Link href="/feed" className="flex items-center gap-2 group">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-black text-white tracking-tight uppercase group-hover:text-primary transition-colors leading-tight">
                  Akış
                </h3>
                <p className="text-[8px] text-neutral-500 font-bold tracking-widest uppercase leading-tight">
                  Arkadaşların
                </p>
              </div>
            </Link>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar relative p-3">
            <Suspense
              fallback={
                <div className="h-48 bg-white/5 motion-safe:animate-pulse rounded-xl" />
              }
            >
