...
export function HomeDiscoverySection({
  ...
  const [activeCategory, setActiveCategory] =
    useState<MenuCategory>("trending");
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        ...
        if (data.genres)
          setGenreOptions([{ id: "", label: "Tür" }, ...data.genres]);
        if (data.providers)
          setProviderOptions([
            { id: "", label: "Platform" },
            ...data.providers,
        ...
        if (data.languages)
          setLanguageOptions([{ id: "", label: "Dil" }, ...data.languages]);
        if (data.countries)
          setCountryOptions([{ id: "", label: "Ülke" }, ...data.countries]);
      } catch (err) {
      ...
      try {
        ...
        if (ratings.length > 0)
          url.searchParams.set(
            "rating",
            Math.min(...ratings.map(Number)).toString(),
          );
        ...
        if (providerItems.length) {
          const providerParams = new URLSearchParams({
            items: providerItems
              .map((item: DiscoverItem) => `${item.media_type}:${item.id}`)
              .join(","),
            country: data.country || "TR",
          });
          const providerResponse = await fetch(
            `/api/tmdb/providers-batch?${providerParams}`,
            { signal: controller.signal },
          );
          if (providerResponse.ok) {
            const providerData = await providerResponse.json();
            if (controller.signal.aborted) return;
            enriched = results.map((item: DiscoverItem) => ({
              ...item,
              watch_providers:
                providerData.providers?.[`${item.media_type}:${item.id}`] ??
                item.watch_providers ??
                null,
            }));
            const updates = new Map<string, DiscoverItem>(
              enriched.map((item: DiscoverItem) => [
            ...
            setItems((previous) =>
              previous.map(
                (item) => updates.get(`${item.media_type}:${item.id}`) || item,
              ),
            );
          }
        }
        if (canCache && page === 1 && !controller.signal.aborted) {
          if (discoveryCache.size >= 40)
  ...
  return (
    <section
      id="home-discover"
      ref={headerRef}
      className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 mt-4 md:mt-5"
    >
      {/* Refined Pro Mobile Filter Overlay */}
      <>
        {isFilterSheetOpen && (
          <div className="fixed inset-0 z-[2000] flex flex-col bg-[#050916] pt-14 lg:hidden animate-in fade-in duration-200">
            {/* Compact Header */}
            ...
            <div className="mx-4 -mt-3 grid grid-cols-3 gap-2 rounded-b-[1.75rem] border-x border-b border-white/10 bg-[#0b1220] p-3 pt-0 shadow-2xl shadow-black/20">
              {[
                ...
                >
                  ...
                  <span
                    className={cn(
                      "mt-0.5 text-[8px] font-bold uppercase tracking-tight",
                      activeFilterCategory === idx
                        ? "text-slate-700"
                        : "text-neutral-600",
                    )}
              ...
              <>
                ...
                >
                  {[
                    {
                      value: stagedGenres,
                      setter: setStagedGenres,
                      options: genreOptions,
                    },
                    {
                      value: stagedProviders,
                      setter: setStagedProviders,
                      options: providerOptions,
                    },
                    {
                      value: stagedYears,
                      setter: setStagedYears,
                    ...
                    {
                      value: stagedRatings,
                      setter: setStagedRatings,
                      options: RATING_OPTIONS,
                    },
                    {
                      value: stagedLanguages,
                      setter: setStagedLanguages,
                      options: languageOptions,
                    },
                    {
                      value: stagedCountries,
                      setter: setStagedCountries,
                      options: countryOptions,
                    },
                  ][activeFilterCategory].options.map(
                    (opt: { id: string; label: string }) => {
                      ...
                      return (
                        <button
                          key={opt.id}
                          onClick={() =>
                            toggleFilter(cat.value, cat.setter, opt.id)
                          }
                          className={cn(
            ...
            <div className="border-t border-white/10 bg-[#090f1c]/95 px-4 py-3 backdrop-blur-2xl">
              <div className="min-h-[42px] rounded-2xl border border-white/8 bg-white/[0.03] px-3 py-2">
                {stagedGenres.length +
                  stagedProviders.length +
                  stagedYears.length +
                  stagedRatings.length +
                  stagedLanguages.length +
                  stagedCountries.length >
                0 ? (
                  <>
                    ...
                    <div className="flex flex-wrap gap-x-2 gap-y-0.5 mt-0.5 max-h-12 overflow-y-auto no-scrollbar">
                      {[
                        ...stagedGenres.map((id) => ({
                          id,
                          label: genreOptions.find((o) => o.id === id)?.label,
                          setter: setStagedGenres,
                          current: stagedGenres,
                        })),
                        ...stagedProviders.map((id) => ({
                          id,
                          label: providerOptions.find((o) => o.id === id)
                            ?.label,
                          setter: setStagedProviders,
                          current: stagedProviders,
                        })),
                        ...stagedYears.map((y) => ({
                          id: y,
                          label: y,
                        ...
                        ...stagedRatings.map((r) => ({
                          ...
                          label: `${r}+ Puan`,
                          setter: setStagedRatings,
                          current: stagedRatings,
                        })),
                        ...stagedLanguages.map((id) => ({
                          id,
                          label: languageOptions.find((o) => o.id === id)
                            ?.label,
                          setter: setStagedLanguages,
                          current: stagedLanguages,
                        })),
                        ...stagedCountries.map((id) => ({
                          id,
                          label: countryOptions.find((o) => o.id === id)?.label,
                          setter: setStagedCountries,
                          current: stagedCountries,
                        })),
                      ]
                        .filter((o) => o.label)
                        .map((opt, i, arr) => (
                  ...
                  <>
                    ...
                    <span className="text-[9px] font-bold text-neutral-500 uppercase tracking-wider truncate">
                      {userPreferences
                        ? [
                            ...userPreferences.genres.map(
                              (id) =>
                                genreOptions.find(
                                  (o) =>
                                    o.id === id ||
                                    o.label.toLowerCase() === id.toLowerCase(),
                                )?.label || id,
                            ),
                            ...userPreferences.platforms.map(
                              (id) =>
                            ...
                            .join(" • ") || "Zevkine uygun içerikler"
                        : "Zevkine uygun içerikler"}
                    </span>
                  </>
                )}
              </div>
              <button onClick={commitFilters} className="hidden">
                Uygula
              </button>
            </div>
          ...
          <div className="lg:w-[320px] shrink-0">
            <div className="flex w-full rounded-[1.35rem] border border-white/10 bg-[#121b2d]/92 p-1 backdrop-blur-xl md:p-1.5 md:rounded-full">
              {["all", "movie", "tv"].map((t) => (
                ...
                >
                  {t === "movie" ? (
                    <Film size={14} className="md:w-[18px] md:h-[18px]" />
                  ) : t === "tv" ? (
                ...
                >
                  ...
                  >
                    <opt.icon
                      ...
                      className={cn(
                        "md:w-3.5 md:h-3.5",
                        activeCategory === opt.id ? "text-amber-400" : "",
                      )}
                    />
                    <span>{opt.label}</span>
                  </div>
            ...
            <div className="flex bg-[#1b2334]/90 p-1.5 rounded-full border border-white/10 backdrop-blur-xl w-full">
              {["day", "week", "month"].map((tw) => (
                <button
                  ...
                  className={cn(
                    "flex-1 flex items-center justify-center px-3 py-2.5 rounded-full text-[10px] md:text-sm font-black transition-all uppercase tracking-tight whitespace-nowrap",
                    activeTimeWindow === tw
            ...
            <div className="flex-1 flex bg-[#1b2334]/90 p-1.5 rounded-full border border-white/10 backdrop-blur-xl gap-0.5 md:gap-1 items-center lg:overflow-visible overflow-x-auto hide-scrollbar">
              {[
                ...
                <FilterDropdown
                  ...
                  onToggle={(val) =>
                    toggleFilter(filter.value, filter.setter, val)
                  }
                />
              ))}
            </div>
          ...
          <div className="hidden lg:flex flex-wrap gap-2 mt-4 animate-in fade-in slide-in-from-top-2 duration-500">
            {getSelectedLabels().map((tag, idx) => (
              <button
                key={`${tag.id}-${idx}`}
                onClick={() =>
                  tag.setter(tag.current.filter((id) => id !== tag.id))
                }
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 md:py-16 px-4 text-center animate-in fade-in zoom-in duration-700 relative z-10">
          ...
          <p className="text-sm md:text-base text-neutral-400 font-bold max-w-md mx-auto leading-relaxed italic">
            &quot;Seçtiğiniz kriterlere uygun hiçbir yapım sinema arşivlerimizde
            eşleşmedi. Farklı kombinasyonlar denemeye ne dersiniz?&quot;
          </p>
          <button
            onClick={() => {
              setGenres([]);
        ...
        <>
          <div
            className={cn(
              ...
              viewMode === "grid" &&
                "relative overflow-visible grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-3 gap-y-5 sm:gap-6",
              viewMode === "compact" &&
                "relative overflow-visible grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-9 gap-x-2 gap-y-4 md:gap-4 justify-items-center",
                ...
                >
                  <MediaCard
                    ...
                    genres={item.genre_ids
                      ?.map(
                        (id: number) =>
                          genreOptions.find((o) => o.id === id.toString())
                            ?.label,
                      )
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 h-full flex flex-col">
                    <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-2">
                      ...
                      {item.genre_ids
                        ?.map((id: number) => {
                          const gLabel = genreOptions.find(
                            (o) => o.id === id.toString(),
                          )?.label;
                          return gLabel ? (
                            <span
                              key={id}
                              className="px-2 py-0.5 rounded-md bg-amber-400/10 border border-amber-400/20 text-[10px] font-black text-amber-400 uppercase"
                            >
                              {gLabel}
                            </span>
                          ) : null;
                    ...
                    <p className="text-sm text-neutral-400 line-clamp-2 md:line-clamp-3 mb-4 font-medium leading-relaxed">
                      {item.overview ||
                        "Bu içerik için henüz bir açıklama eklenmemiş."}
                    </p>

                    {/* Watch Providers */}
                    {item.watch_providers?.flatrate && (
                      <div className="flex items-center gap-2 mt-auto">
                        <span className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mr-1">
                          Platformlar:
                        </span>
                          ...
                          {item.watch_providers.flatrate
                            ...
                            .map((provider) => (
                              <div
                                ...
                                className="w-6 h-6 rounded-md overflow-hidden border border-white/10 tooltip"
  ...
  return (
    <div className="relative shrink-0 flex-1 min-w-[80px]" ref={dropdownRef}>
      ...
      >
        {selected.length > 0 ? (
          <>
            <span className="text-[8px] md:text-[11px] mb-0.5">
              {selected.length} {label}
            </span>
            <span className="text-[7px] md:text-[9px] opacity-70 font-bold">
              Seçili
            </span>
          </>
        ) : (
