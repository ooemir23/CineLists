...
const CATEGORY_CONFIG = {
  ...
  popular: {
    label: "Haftanın Filmi",
    icon: Trophy,
    color: "text-purple-400",
    bgColor: "bg-purple-400",
  },
  personalized: {
    label: "Size Özel Öneri",
    icon: Trophy,
    color: "text-rose-400",
    bgColor: "bg-rose-400",
  },
...
> = {
  ...
  "Platformda Yayinda": {
    label: "Platformda Yayında",
    icon: TrendingUp,
    color: "text-cyan-400",
    bgColor: "bg-cyan-500/10",
  },
  "Platformunda Yeni": {
    label: "Platformunda Yeni",
    icon: TrendingUp,
    color: "text-cyan-400",
    bgColor: "bg-cyan-500/10",
  },
...
export function HeroSlider({ items }: HeroSliderProps) {
  ...
  const eventMeta = currentItem.eventLabel
    ? EVENT_LABELS[currentItem.eventLabel]
    : null;
  ...
  return (
    ...
    >
      <div
        key={currentItem.id}
        className="absolute inset-0 animate-hero-fade-scale"
      >
        <Image
          src={backdropUrl}
          alt=""
        ...
        <div className="absolute inset-0 z-20 flex flex-col p-4 pt-4 md:pt-5 lg:pt-6 md:p-8 lg:p-10">
          ...
          >
            <div className="flex flex-wrap gap-2 items-center">
              ...
              >
                <categoryConfig.icon
                  className={cn("w-3 h-3", categoryConfig.color)}
                />
            ...
            <div className="flex items-center gap-3">
              {currentItem.platformLogos &&
                currentItem.platformLogos.length > 0 && (
                  <div className="hidden md:flex items-center gap-1.5 bg-black/40 backdrop-blur-xl px-3 py-1.5 rounded-full border border-white/10 transition-all hover:bg-black/60 cursor-pointer">
                    <span className="text-[9px] font-black text-white/40 uppercase tracking-widest mr-1">
                      İzle:
                    </span>
          ...
          <div className="flex-1 flex flex-col justify-end mb-8 md:mb-4 lg:mb-6">
            <div className="flex flex-col items-start gap-4 md:gap-6">
              <div className="max-w-3xl space-y-3 md:space-y-4">
                <Link
                  href={`/${currentItem.media_type}/${currentItem.id}`}
                  className="space-y-2 block group/content"
                >
                  <h2
