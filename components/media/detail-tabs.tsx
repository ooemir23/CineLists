...
export function DetailTabs({
  ...
  const [recommendationsLoaded, setRecommendationsLoaded] = useState(
    ...
    const loadRecommendations = async () => {
      ...
      try {
        const res = await fetch(
          `/api/tmdb/recommendations?type=${type}&id=${tmdbId}`,
          {
  ...
  return (
    <div className="space-y-3 sm:space-y-4 md:space-y-8">
        ...
        {activeTabs.map((tab) => (
          ...
          >
            <tab.icon
              ...
              className={cn(
                "shrink-0",
                activeTab === tab.id ? "text-slate-950" : "text-neutral-500",
              )}
            />
            {tab.label}
            {tab.id === "cast" && cast.length > 0 && (
              <span
                className={cn(
                  "ml-0.5 px-1.5 py-0.2 rounded-md text-[8px] md:text-[9px] font-bold",
                  activeTab === tab.id ? "bg-slate-950/20" : "bg-white/5",
                )}
              >
                {cast.length}
              </span>
      ...
      <div className="relative animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {activeTab === "heatmap" && (
          <TvHeatmap tmdbId={tmdbId} seasons={seasons || []} />
        )}
