...
const WatchedSearchBar: React.FC<WatchedSearchBarProps> = ({
  ...
  <div className="flex flex-col md:flex-row items-center w-full bg-[#1b2334]/60 backdrop-blur-xl rounded-2xl md:rounded-[2rem] px-4 py-3 md:py-2 border border-white/5 shadow-2xl gap-3 md:gap-4">
    {/* Arama kutusu */}
    <div className="flex items-center flex-1 bg-white/5 rounded-xl md:rounded-2xl px-4 py-2.5 border border-white/5 group focus-within:border-amber-400/30 transition-all w-full">
      <Search size={18} className="text-neutral-500 group-focus-within:text-amber-400 transition-colors mr-3 shrink-0" />
      <input
        type="text"
        value={searchValue}
        onChange={e => onSearchChange(e.target.value)}
        placeholder="Listenizde arama yapın..."
    ...
    <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-end shrink-0">
      <div className="flex bg-white/5 p-1 rounded-xl md:rounded-2xl border border-white/5">
        <button
          className={cn(
            "p-2.5 rounded-lg md:rounded-xl transition-all",
      ...
      <button
        className={cn(
          "flex items-center gap-2 px-5 py-2.5 rounded-xl md:rounded-2xl transition-all font-black text-xs uppercase tracking-wider border border-white/5 active:scale-95",
