...
export function PortalTrendingTopics({
  trendingTopics,
}: {
  trendingTopics: TrendingTopicItem[];
}) {
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="w-5 font-black text-sm text-neutral-500 group-hover:text-amber-400 transition-colors shrink-0">
                      #{topic.rank}
                    </span>
                    <strong className="text-sm font-bold text-neutral-200 group-hover:text-white transition-colors truncate">
                      {topic.title}
                    </strong>
                  </div>
                  <span
                    className={`text-[10px] font-black uppercase px-1.5 py-0.5 rounded border shrink-0 ${tagColor}`}
                  >
                    {topic.heatStatus}
                  </span>
                </div>
            ...
            >
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-sm font-bold text-neutral-200 group-hover:text-amber-400 transition-colors truncate">
                    {rec.title}
                  </p>
                  <small className="text-xs text-neutral-400 block mt-0.5">
                    {rec.category}
                  </small>
                </div>
              </Link>
...
export function PortalWeeklyPoll() {
  ...
  return (
    <section className="p-4 sm:p-5 rounded-2xl md:rounded-3xl border border-white/10 bg-slate-900/60 backdrop-blur-md">
      ...
      <form onSubmit={handlePollVote} className="space-y-3">
        <h3 className="text-sm font-bold text-neutral-200 leading-snug">
