"use client";

import { useState } from "react";
import { signIn } from "next-auth/react";
import { loginAsGuest } from "@/lib/guest-actions";

type SocialAuthProps = {
    showGuest?: boolean;
};

export function SocialAuth({ showGuest }: SocialAuthProps) {
    const [googleLoading, setGoogleLoading] = useState(false);
    const [googleError, setGoogleError] = useState<string | null>(null);

    const handleGoogleSignIn = async () => {
        setGoogleLoading(true);
        setGoogleError(null);
        try {
            const result = await signIn("google", { callbackUrl: "/onboarding", redirect: false });
            if (result?.error) {
                setGoogleError("Google ile giriş başarısız. Lütfen tekrar deneyin.");
                setGoogleLoading(false);
            } else if (result?.ok && result?.url) {
                window.location.href = result.url;
            } else {
                // redirect: false ile redirect URL gelmezse manuel yönlendirme
                window.location.href = "/onboarding";
            }
        } catch (error) {
            console.error("Google sign-in error:", error);
            setGoogleError("Bir hata oluştu. Lütfen tekrar deneyin.");
            setGoogleLoading(false);
        }
    };

    const handleMailClick = () => {
        const emailInput = document.getElementById("email");
        if (emailInput) {
            emailInput.focus();
            emailInput.scrollIntoView({ behavior: "smooth", block: "center" });
        }
    };

    const gridCols = showGuest ? "grid-cols-3" : "grid-cols-2";

    return (
        <div className={`grid ${gridCols} gap-3`}>
            {googleError && (
                <div className="col-span-full text-xs text-red-400 text-center bg-red-500/10 border border-red-400/20 rounded-lg py-2 px-3">
                    {googleError}
                </div>
            )}
            <button
                onClick={handleGoogleSignIn}
                disabled={googleLoading}
                className="w-full flex flex-col items-center justify-center gap-1 bg-white/5 text-white py-3 rounded-xl hover:bg-white/10 transition-colors border border-white/10 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                {googleLoading ? (
                    <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : (
                    <svg className="w-5 h-5" viewBox="0 0 24 24">
                        <path
                            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                            fill="#4285F4"
                        />
                        <path
                            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                            fill="#34A853"
                        />
                        <path
                            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                            fill="#FBBC05"
                        />
                        <path
                            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 2.09 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                            fill="#EA4335"
                        />
                    </svg>
                )}
                <span className="text-[10px] font-semibold uppercase opacity-60">{googleLoading ? "Yönlendiriliyor..." : "Google"}</span>
            </button>

            <button
                onClick={handleMailClick}
                className="w-full flex flex-col items-center justify-center gap-1 bg-white/5 text-white py-3 rounded-xl hover:bg-white/10 transition-colors border border-white/10"
            >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <rect width="20" height="16" x="2" y="4" rx="2" />
                    <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
                </svg>
                <span className="text-[10px] font-semibold uppercase opacity-60">Mail</span>
            </button>

            {showGuest && (
                <button
                    onClick={() => loginAsGuest()}
                    className="w-full flex flex-col items-center justify-center gap-1 bg-white/5 text-white py-3 rounded-xl hover:bg-white/10 transition-colors border border-white/10"
                >
                    <span className="text-lg leading-none">👻</span>
                    <span className="text-[10px] font-semibold uppercase opacity-60">Misafir</span>
                </button>
            )}
        </div>
    );
}
